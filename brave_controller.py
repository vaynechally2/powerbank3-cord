...
class BraveController:
    ...
    def default_brave_path() -> str:
        ...
        candidates.append(Path(local) / "BraveSoftware/Brave-Browser/Application/brave.exe")
        candidates.append(Path(pf) / "BraveSoftware/Brave-Browser/Application/brave.exe")
        candidates.append(Path(pfx86) / "BraveSoftware/Brave-Browser/Application/brave.exe")
        for c in candidates:
            if c.exists():
                return str(c)
    ...
    def start(self, url: str, brave_executable: Optional[str] = None) -> None:
        brave_path = (brave_executable or "").strip() or self.default_brave_path()
        if not Path(brave_path).exists():
            raise FileNotFoundError(
                "Brave was not found. Set Brave path in the app, or install Brave in the default folder."
            )
