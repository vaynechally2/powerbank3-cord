...
class MonitorEngine:
    ...
    def _run(self) -> None:
        try:
            self.controller.start(self.profile.target_url, self.profile.brave_path)
            line = self.logger.log("STARTED", "Monitoring started", self.profile.clock_offset_seconds, self.profile.log_csv)
            self.callbacks.on_log(line)
            self.callbacks.on_status("Running")
            self.callbacks.on_detection("RUNNING")

            planned = max(0, self.profile.startup_verification_refreshes)
            if planned > 0:
                line = self.logger.log(
                    "STARTUP_VERIFY_BEGIN",
                    f"planned_refreshes={planned}",
                    self.profile.clock_offset_seconds,
                    self.profile.log_csv,
                )
                self.callbacks.on_log(line)
                for i in range(1, planned + 1):
                    self.callbacks.on_status(f"Startup check {i}/{planned}")
                    self.callbacks.on_detection(f"Startup check {i}/{planned}")
                    title = self.controller.refresh()
                    line = self.logger.log(
                        "REFRESH_STARTUP",
                        f"pass={i}/{planned} | title={title}",
                        self.profile.clock_offset_seconds,
                        self.profile.log_csv,
                    )
                    self.callbacks.on_log(line)
                    time.sleep(max(0.1, self.profile.startup_verification_delay_ms / 1000.0))
                self.callbacks.on_status("Startup verification complete")
                self.callbacks.on_detection("Startup verification complete")
                self.callbacks.on_log(
                    self.logger.log(
                        "STARTUP_VERIFY_COMPLETE",
                        "Startup verification completed; entering normal schedule.",
                        self.profile.clock_offset_seconds,
                        self.profile.log_csv,
                    )
                )
